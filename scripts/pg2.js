...
function setup() {
    ...
    /*
    timeSlider = s.createSlider(1,50,50);
    timeSlider.position(600,150);
    timeSlider.style("color","#0E406F");
    timeSlider.size(buttonWidth-50,buttonHeight);
    timeSliderElem = s.createDiv(Math.floor(timeSlider.value()));
    timeSliderElem.id('slider');
    timeSliderElem.style('color','black');
    timeSliderElem.style('font-size','28px');
    timeSliderElem.position(775,157);
    */
    createCanvas(width + 300, height);
    frameRate(3); //code in draw gets run once per sec
    background(255);
    textSize(28);
...
function resetClick() {
    ...
    //testroom.testCreate1Ped(Math.floor(mouseX/rectwidth),Math.floor(mouseY/rectheight));
    testroom.setupRoom();
    resetCanvas();
}
function startClick() {
    if (run === false && testroom.occupiedCells.length > 0) {
        frame = 1;
        run = true;
        loop();
    }
...
function draw() {
    ...
    if (frame % 3 === 0) {
        ...
        //Display neighbours and runs reserve() function
        for (let i = 0; i < testroom.occupiedCells.length; i++) {
            let selectedNeighbours = testroom.occupiedCells[i].neighbours;
            for (let h = 0; h < selectedNeighbours.length; h++) {
                fill(255, 204, 0); //Fill with yellow
                rect(
                    selectedNeighbours[h].x * rectwidth,
                    selectedNeighbours[h].y * rectheight,
                    rectwidth,
        ...
        //Display reserved cells and runs move() function
        for (let i = 0; i < testroom.reservedCells.length; i++) {
            fill(144, 238, 144); //Fill with light green
            rect(
                testroom.reservedCells[i].x * rectwidth,
                testroom.reservedCells[i].y * rectheight,
                rectwidth,
                rectheight
            );
        }
